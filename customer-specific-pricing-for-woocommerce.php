<?php
/**
 * Plugin Name: Customer Specific Pricing for WooCommerce wisdmLabs
...
if (!defined('CSP_PLUGIN_PATH')) {
	define('CSP_PLUGIN_PATH', untrailingslashit(plugin_dir_path(__FILE__)));
	if (!defined('CSP_PLUGIN_URL')) {
		define('CSP_PLUGIN_URL', CSP_PLUGIN_PATH); //for backward compatibility with customizations
	}
}

if (!defined('CSP_PLUGIN_SITE_URL')) {
	/**
	 * The constant CSP_PLUGIN_SITE_URL contains the url path to the plugin directory
	 * eg. https://example.com/wp-content/plugins/customer-specific-pricing-for-woocommerce/
	 */
	define('CSP_PLUGIN_SITE_URL', untrailingslashit( plugins_url( '/', CSP_PLUGIN_FILE ) ));
}
 
if (!defined('CSP_VERSION')) {
	define('CSP_VERSION', '4.6.17.3');
}
...
function cspTextInitFunc() {
	...
	if (isset($cspSettings['csp_api_status']) && 'enable' == $cspSettings['csp_api_status']) {
		include_once CSP_PLUGIN_PATH . '/includes/csp-api/csp-api.php';	
	}
...
require_once CSP_PLUGIN_PATH . '/includes/single-view/class-wdm-subrule-management.php';

require_once CSP_PLUGIN_PATH . '/includes/single-view/class-wdm-rule-management.php';

//Including a class having common CSP functions
require_once CSP_PLUGIN_PATH . '/includes/class-wdm-wusp-functions.php';
...
if (version_compare($php_version, '5.6.0') >= 0) {
	...
	include CSP_PLUGIN_PATH . '/includes/class-wdm-wusp-install.php';

	// This hook will add beacond icon on backend page.
	add_action('customer_specific_pricing_backend_page', 'add_beacon_helpscout_script');
	...
	if ($cspFunctions->wdmIsActive('woocommerce/woocommerce.php')) {
		...
		add_action('admin_notices', 'cspLicenseNotice', 1);
		add_action('plugins_loaded', 'cspUpdateMeta');
		add_action('plugins_loaded', 'cspTextInitFunc');

		global $wdmPluginDataCSP;

		include_once CSP_PLUGIN_PATH . '/includes/class-wdm-wusp-add-data-in-db.php';
		include_once CSP_PLUGIN_PATH . '/includes/class-wdm-wusp-delete-data.php';
		include_once CSP_PLUGIN_PATH . '/includes/class-wdm-wusp-update-data.php';
...
function add_beacon_helpscout_script () {
	...
	</script><script type="text/javascript">window.Beacon('init', '92227d42-533e-4ca3-a21c-7324bb32af31')</script>
	<?php
}
...
function cspBootstrap() {
	if (is_user_logged_in()) {
		//Global Discounts
		include_once CSP_PLUGIN_PATH . '/includes/global-discount/class-global-discount-application.php';
		new \cspGlobalDiscounts\GlobalDiscountApplication();

		//including file for the working of Customer Specific Pricing on Simple Products
		include_once CSP_PLUGIN_PATH . '/includes/user-specific-pricing/class-wdm-wusp-simple-products-usp.php';
		...
		include_once CSP_PLUGIN_PATH . '/includes/user-specific-pricing/class-wdm-wusp-variable-products-usp.php';
		...
		include_once CSP_PLUGIN_PATH . '/includes/group-specific-pricing/class-wdm-wusp-simple-products-gsp.php';
		...
		include_once CSP_PLUGIN_PATH . '/includes/group-specific-pricing/class-wdm-wusp-variable-products-gsp.php';
		...
		include_once CSP_PLUGIN_PATH . '/includes/role-specific-pricing/class-wdm-wusp-simple-products-rsp.php';
		...
		include_once CSP_PLUGIN_PATH . '/includes/role-specific-pricing/class-wdm-wusp-variable-products-rsp.php';
		...
		include_once CSP_PLUGIN_PATH . '/includes/class-wdm-usp-product-price-commons.php';
		...
		include_once CSP_PLUGIN_PATH . '/includes/dashboard-orders/class-wdm-customer-specific-pricing-new-order.php';
		new cspNewOrder\WdmCustomerSpecificPricingNewOrder();
		include_once CSP_PLUGIN_PATH . '/includes/class-wdm-single-view-tabs.php';
		new SingleView\WdmShowTabs();
		
		include_once CSP_PLUGIN_PATH . '/includes/class-wdm-product-tables.php';
		new WdmCSP\WdmCspProductTables();
		//CSP-PEP Compatibility
		include_once CSP_PLUGIN_PATH . '/includes/compatibility/class-wdm-pep.php';
		new WdmCSP\Compat\WisdmPEP();

	}

	include_once CSP_PLUGIN_PATH . '/includes/class-wdm-wusp-ajax.php';
	new cspAjax\WdmWuspAjax();
	
	include_once CSP_PLUGIN_PATH . '/includes/products-archive/class-product-archive.php';
	new CSPProductArchive\CustomerSpecificProductArchive();
	
	include_once CSP_PLUGIN_PATH . '/includes/cart-discount/class-wdm-csp-cd-application.php';
 ...
 *
 * @since 2.3.1
 * @param array $upgraderObject contain details for the recently completed upgrade process
 * @param array $options - list of upgradable entities.
 * @return void
 */
...
if (!function_exists('cspUpdateMeta')) {
	function cspUpdateMeta() {
		...
		if (!$cspMeta) {
			...
			/**
			 * This action gets executed before upgrading CSP to the newest version.
			 *
			 * @since 1.0.0
			 * @param array $cspMeta {Array containg the following information about the plugin.  
			 * 		@type string $siteURL Website URL Where the upgrade attempt is being made.
			 * 		@type string $cspVersion Currently installed version of CSP.
			 *  	@type string installTime The time entry of last CSP update or the first install.
			 * }
			 */
			...
			update_option('wisdmCSPMeta', $cspMeta);
			/**
			 * This action gets executed after upgrading CSP to the newest version.
			 *
			 * @since 1.0.0
			 * @param array $cspMeta {Array containg the following information about the plugin.  
			 * 		@type string $siteURL Website URL Where the upgrade is attempted.
			 * 		@type string $cspVersion Current CSP version.
			 *  	@type string installTime Last Upgrade time.
			 * }
			 */
...
if (!function_exists('csp_dropdown_roles')) {
	/**
	 * Gets all the editable rules for the user , genetartes 
	 * HTML select box (dropdown) content for the user roles
	 * Returns the for the user role dropdown 
	 * 
	 * @since 4.6.0
	 * @param string $selected slug of the pre-selected user-role 
	 * @return string html select box
	 */
	function csp_dropdown_roles( $selected = '') {
		$roleDropDown 				= '';
...
//  This action will declare HPOS compatible.
