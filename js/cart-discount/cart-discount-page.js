...
jQuery( document ).ready(function() {
    ...
    jQuery("#csp-cd-search").on("keyup", function() {
        ...
        jQuery(document).on('change', 'input, select', function (event) {
            if ('csp-cd-feature-switch'!=event.currentTarget.id) {
                changesMade = true;    
            }
        });
        ...
        jQuery(window).bind('beforeunload', function(){
            if (changesMade) {
                return wdm_csp_cd_object.window_close_confirm_message;
            }
        });
}); //End Document ready
...
// File: plugins/csp/js/cart-discount/cart-discount-page.js
function cspIsValid(url) {
    // Accepts absolute URLs and relative URLs starting with /
    return /^(https?|s?ftp):\/\/.+/.test(url) || /^\//.test(url);
}
