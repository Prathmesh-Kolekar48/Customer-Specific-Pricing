jQuery(document).ready(function () {
    ...
    var user_row = jQuery( 'div.user-row' ).length + 1;
    var role_row = jQuery( 'div.role-row' ).length + 1;
    var group_row = jQuery( 'div.group-row' ).length + 1;
    ...
    */
    ...
    jQuery('div.csp-ct-main-div').on('keypress', '.remove_user_row_image, .remove_role_row_image, .remove_group_row_image, .add_new_user_row_image, .add_new_role_row_image, .add_new_group_row_image', function(event){
    ...
    jQuery(document).on('change', 'input, select', function () {
        if ('csp-cat-feature-switch'==jQuery(this).attr('id')) {
            return ;    
        }
        changesMade = true;
    });
    ...
    jQuery(window).bind('beforeunload', function(){
        if (changesMade) {
            return cat_pricing_object.window_close_confirm_message;    
        }
      });

    /**
     * Changes the chnanges made to false while saving the rules.
     */
    jQuery('#cat_pricing').click(function(){
        changesMade = false;
    });
});
