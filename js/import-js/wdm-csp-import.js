...
function senddata(filename, type, batchNumber, uniqueId){
    ...
    var file     = filename;
    var fileType = type;
    var action   = getCSPImportActionfrom(fileType);
...
/**
 * This file finds the ajax callback actiion for the import file type
 * and returns the action paramter for the ajax request.
 * 
 * @param fileType 
 */
function getCSPImportActionfrom(fileType) {
    let action = '';
    switch (fileType) {
        case 'role':
...
jQuery( 'document' ).ready( function ( jQuery ) {
    ...
    jQuery( '#dd_show_import_options' ).on( "change", function () {
        ...
        jQuery( '#wdm_message' ).hide();
        let importFor   = jQuery('input[name="wdm_csp_import_rules_for"]:checked').val();
        let importUsing = importFor;
        if ("Products"==importFor) {
             importUsing = jQuery("input[name='wdm_csp_import_using']:checked").val() == 'sku' ? jQuery("input[name='wdm_csp_import_using']:checked").val() : 'product_id';            
        }
        setCSVTemplateUrl(jQuery(this).val() + '_' + importUsing);
    } );

    jQuery("input.wdm_csp_import_using, input.wdm_csp_import_rules_for").on("change", function(){
        let importFor   = jQuery('input[name="wdm_csp_import_rules_for"]:checked').val();
        let importUsing = importFor;
        if ("Products"==importFor) {
             importUsing = jQuery("input[name='wdm_csp_import_using']:checked").val() == 'sku' ? jQuery("input[name='wdm_csp_import_using']:checked").val() : 'product_id';            
        }
        let importType  = jQuery('#dd_show_import_options').val();
    ...
    */
    function setCSVTemplateUrl(importType) {
        switch(importType) {
            ...
            case 'Wdm_User_Specific_Pricing_Import_Categories':
                jQuery('.sample-csv-import-template-link').attr('href', wdm_csp_import.templates_url + 'category-pricing-rules-for-users-sample.csv');
                jQuery('span.import-type').text(wdm_csp_import.user_specific_sample);
                break;

            case 'Wdm_Role_Specific_Pricing_Import_Categories':
                jQuery('.sample-csv-import-template-link').attr('href', wdm_csp_import.templates_url + 'category-pricing-rules-for-roles-sample.csv');
                jQuery('span.import-type').text(wdm_csp_import.role_specific_sample);
                break;

            case 'Wdm_Group_Specific_Pricing_Import_Categories':
                jQuery('.sample-csv-import-template-link').attr('href', wdm_csp_import.templates_url + 'category-pricing-rules-for-groups-sample.csv');
                jQuery('span.import-type').text(wdm_csp_import.group_specific_sample);
                break;

            case 'Wdm_User_Specific_Pricing_Import_GlobalDiscounts':
                jQuery('.sample-csv-import-template-link').attr('href', wdm_csp_import.templates_url + 'global-pricing-rules-for-users-sample.csv');
                jQuery('span.import-type').text(wdm_csp_import.user_specific_sample);
                break;

            case 'Wdm_Role_Specific_Pricing_Import_GlobalDiscounts':
                jQuery('.sample-csv-import-template-link').attr('href', wdm_csp_import.templates_url + 'global-pricing-rules-for-roles-sample.csv');
                jQuery('span.import-type').text(wdm_csp_import.role_specific_sample);
                break;

            case 'Wdm_Group_Specific_Pricing_Import_GlobalDiscounts':
                jQuery('.sample-csv-import-template-link').attr('href', wdm_csp_import.templates_url + 'global-pricing-rules-for-groups-sample.csv');
                jQuery('span.import-type').text(wdm_csp_import.group_specific_sample);
                break;
    ...
    jQuery('.csp-download-report').click(function() {
     ...
     function download(dataurl, filename) {
      ...
      jQuery('input.wdm_csp_import_rules_for').click(function () {
          let selection = jQuery(this).val();
          switch (selection) {
              case 'Products':
                  jQuery('div.product-import-options').show(500);
                  break;
              default:
                jQuery('div.product-import-options').hide(500);
                  break;
          }
      });
     ...
     */
    jQuery('div.import-notes-title').click(function(){
        $content = jQuery(this).next('.import-notes-content');
