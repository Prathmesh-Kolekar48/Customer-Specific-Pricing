jQuery(document).ready(function () {
    ...
    jQuery('#woocommerce-order-items' ).on('quantity_changed', 'input.quantity', function(data){
        ...
        if ("undefined" == typeof(quantityBasedPricing[user_id][product_id])) {
            return;
        }
        ...
        let regular_price = quantityBasedPricing[user_id][product_id]['regular_price'];
        
        if (wdm_new_order_ajax.allow_custom_price_for_wc_admin_order_item) {
            let current_price = jQuery(this).closest('tr.item').find('td.item_cost').attr('data-sort-value');   
            let current_qty   = jQuery(this).attr('data-qty');
            let is_current_price_custom = false;
            is_current_price_custom = current_price_custom(current_price, current_qty, quantityBasedPricing[user_id][product_id]);
            if (is_current_price_custom) {
                return  ;
            }
        }
    ...
    /**
     * This function checks wether the custom price is added to the admin order item
     * based on the existing price of the item & the list of CSP prices available for that item.
     * 
     *  
     * @param float current_price Existing price of the item in the order
     * @param int current_qty exiting quantity of the item in the order
     * @param array csp_prices array of CSP prices & quantities for the item for the user.
     */
    function current_price_custom(current_price, current_qty,  csp_prices) {
        if (!empty(csp_prices)) {
            let regular_price          = csp_prices['regular_price'];
            let current_applicable_csp = getApplicablePrice(csp_prices['qtyList'], current_qty, csp_prices['csp_prices'], regular_price);
            if (current_price!=current_applicable_csp) {
                return true;
            }
        }
        return false;
