...
//Elements for groups tab added.
    ...
    var i = jQuery('#wdm_group_specific_pricing_tbody tr').length + 1;

    // This trigger to dynamically add a new row when focused on the last 'value' input field. As adding a new row function does not exist, for time being, triggering a click event on the 'plus' button.
    jQuery('body').on('focus', '#wdm_group_specific_pricing_tbody .wdm_price:last', function() {
        jQuery( this ).closest('tbody').find( '#wdm_add_new_group_price_pair' ).click();
    });
    ...
    jQuery('#group_specific_pricing_tab_data').delegate('#wdm_add_new_group_price_pair', 'click', function () {
        ...
        if (!jQuery(this).prop('type')=='button' || jQuery(this).prop('type')==undefined) {
