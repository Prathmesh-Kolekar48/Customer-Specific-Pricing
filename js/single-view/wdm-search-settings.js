...
(function( $ ) {
	 ...
	 */
	 ...
	 $(function() {
		...
		jQuery('input[name="price-for-select"]').click(function(){
			let selection = jQuery(this).prop('id');
				switch (selection) {
					case 'customer':
						displaySection('div.row-customer-selection');
						break;
					case 'role':
						displaySection('div.row-user-role-selection');
						break;
					case 'group':
						displaySection('div.row-user-group-selection');
						break;
					default:
						break;
				}
		...
		function displaySection(section) {
			jQuery('div.row-selection').hide(500);
			jQuery('.wdm-selection-price-list-wrapper').remove();
			jQuery('input.wdm-remove-sel-csp-price').hide();
			jQuery(section).find('select').val(null).trigger('change');
			jQuery(section).show(500);
		}

		function fetchCSPData(option_type, selection_name) {
			$('input.wdm-remove-sel-csp-price').hide();
			...
			if(selection_name != -1)
			{
				$('div.wdm-csp-single-view-search-wrapper').after('<img src="' + single_view_obj.loading_image_path + '" id="loading"/>');
		...
		$('input#remove-sel-csp-price').click(function(){
			...
			var option_type    	= jQuery('input[name="price-for-select"]:checked').prop('id');

			var selection_name 	= jQuery('div.row-selection:visible').find('select').val();
			...
			jQuery.ajax({
				...
				success: function (response) {//response is value returned from php
					$('#loading').remove();
					let selection = jQuery('div.row-selection:visible').find('select').val();
					let option_type = jQuery('input[name="price-for-select"]:checked').prop('id');
					fetchCSPData(option_type ,selection);
				}
			});
		...
		jQuery('select.user-role-selection').select2({
			placeholder: csp_settings_data.search_select_text
		  });

		jQuery('select.user-group-selection').select2({
			placeholder: csp_settings_data.search_select_text
		});

		jQuery('select.user-selection').select2({
			placeholder: csp_settings_data.search_select_text,
			ajax: {
				url: single_view_obj.admin_ajax_path,
				dataType: 'json',
				...
				data : function (params) {
					var query = {
						...
						action:'csp_get_users_by_search_term',
						nonce: jQuery('#search-by-page-nonce').val(),
					  }
					return query;
				  },
		...
		jQuery('select.user-selection').on('select2:select', function (e) {
			var data = e.params.data;
			var option_type = 'customer'
			var selection_name = data.id;
			fetchCSPData(option_type, selection_name);
		});

		jQuery('select.user-role-selection').on('select2:select', function (e) {
			var data = e.params.data;
			var option_type = 'role'
			var selection_name = data.id;
			fetchCSPData(option_type, selection_name);
		});

		jQuery('select.user-group-selection').on('select2:select', function (e) {
			var data = e.params.data;
			var option_type = 'group'
			var selection_name = data.id;
			fetchCSPData(option_type, selection_name);
		});
