...
if (empty($array_of_groupname_price_pair)) {
	...
	$wdm_first_price_of_group = wc_format_localized_price(wc_format_decimal($array_of_groupname_price_pair[0]->price, '', true));
...
if (is_array($array_of_groupname_price_pair) && count($array_of_groupname_price_pair) > 0 && false != $array_of_groupname_price_pair) {
	...
	<script type="text/javascript">
		...
		if ( 2 == $array_of_groupname_price_pair[ $j ]->price_type ) {
			...
			?>
	price_textbox = "<td colspan=3 class='wdm_left_td'><input type='text' name='wdm_woo_group_price[]' class='wdm_price csp-percent-discount' value='<?php echo esc_attr(wc_format_localized_price(wc_format_decimal($array_of_groupname_price_pair[ $j ]->price, '', true))); ?>' /></td><td><a class='wdm_remove_pair_link' href='#' id='group_remScnt'  >";
			<?php
		} else {
			//Show Price's Textbox
			?>
	price_textbox = "<td colspan=3 class='wdm_left_td'><input type='text' name='wdm_woo_group_price[]' class='wdm_price' value='<?php echo esc_attr(wc_format_localized_price(wc_format_decimal($array_of_groupname_price_pair[ $j ]->price, '', true))); ?>' /></td><td><a class='wdm_remove_pair_link' href='#' id='group_remScnt'  >";
			<?php
		}
		?>
			//Show Price's Textbox
