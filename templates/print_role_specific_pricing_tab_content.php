...
if (is_array($array_of_role_price_pair) && count($array_of_role_price_pair) > 0 && false != $array_of_role_price_pair) {
	...
	for ($j = 1; $j < count($array_of_role_price_pair); $j ++) {
		...
		$wdm_roles_dropdown = csp_dropdown_roles($array_of_role_price_pair[ $j ]->role);
		echo '
		...
		if (2 == $array_of_role_price_pair[ $j ]->price_type) {
			echo "price_textbox = \"<td colspan=3 class='wdm_left_td'><input type='text' name='wdm_woo_role_price[]' class='wdm_price csp-percent-discount' value='" . esc_attr(wc_format_localized_price(wc_format_decimal( $array_of_role_price_pair[ $j ]->price, '', true))) . "' />\";";
		} else {
			echo "price_textbox = \"<td colspan=3 class='wdm_left_td'><input type='text' name='wdm_woo_role_price[]' class='wdm_price' value='" . esc_attr(wc_format_localized_price(wc_format_decimal($array_of_role_price_pair[ $j ]->price, '', true))) . "' />\";";
		}
	...
	$wdm_roles_dropdown   = csp_dropdown_roles($roleSelected);
	$price                     = wc_format_localized_price(wc_format_decimal($array_of_role_price_pair[0]->price, '', true));
	...
	$wdm_roles_dropdown   = csp_dropdown_roles();	
}
